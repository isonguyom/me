  ...
  >
    ...
    >
      <img
        ...
        loading="lazy"
      />
      <div class="absolute inset-0 bg-black/20 flex items-end pb-4 p-4 md:px-6"></div>
    </div>
    ...
    >
      <div>
        ...
        <h3
          ref="titleRef"
          class="text-xl md:text-3xl lg:text-5xl font-bold uppercase text-secondary lg:mt-2"
        >
          {{ project.title }}
        </h3>
      </div>
      ...
      <p
        ref="descRef"
        class="text-sm md:text-base lg:text-xl lg:pr-6 text-pretty opacity-80 mb-2 lg:mb-6"
      >
        {{ project.description }}
      </p>
...
function animateIn() {
  ...
  if (buttonRef.value)
    tl.from(buttonRef.value.$el || buttonRef.value, { opacity: 0, y: 30 }, "-=0.4");
