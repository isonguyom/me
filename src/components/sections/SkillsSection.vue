<template>
  ...
  >
    ...
    <div
      ref="skillsContent"
      class="relative z-10 w-full h-full px-4 md:px-6 xl:px-10 max-w-4xl mx-auto"
    >
      <!-- Tabs -->
      <div ref="tabsRef" class="flex flex-wrap gap-2 mb-4 py-2 w-full">
        <button
          ...
          class="px-4 py-1 font-medium transition duration-300 cursor-pointer min-w-max text-sm md:text-base"
          :class="
            activeTab === index
      ...
      <div ref="skillsBox" class="bg-white text-text p-6 shadow-lg space-y-2 md:space-y-4">
        ...
        <ul
          class="list-disc list-inside space-y-1 text-lg md:text-xl"
          :id="`panel-${activeTab}`"
          role="tabpanel"
        >
          <li
            v-for="(item, i) in skills[activeTab].skills"
            :key="item"
            :ref="(el) => (skillItems[i] = el)"
          >
            {{ item }}
          </li>
        </ul>
